...
# ================== SaaS 多租户扩展支持 ==================
def get_project_key_paths(project_id):
    """
    根据项目 ID 动态获取该项目的专属密钥对文件路径
    """
    base_dir = "myrsa_test"
    private_path = os.path.join(base_dir, f"project_{project_id}_private.pem")
    public_path = os.path.join(base_dir, f"project_{project_id}_public.pem")
    return private_path, public_path


def generate_project_keys(project_id):
    """
    为特定租户(项目)生成专属的 RSA 密钥对，并按 ID 命名保存
    """
    os.makedirs("myrsa_test", exist_ok=True)  # 确保文件夹存在
    private_path, public_path = get_project_key_paths(project_id)
...
def test():
    ...
    priv_key, pub_key = generate_rsa_keys(
        "./myrsa_test/alice_private.pem", "./myrsa_test/alice_public.pem"
    )
